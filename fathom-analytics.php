...
function fathom_get_url() {
	...
	// make relative
	return str_replace( array( 'https:', 'http:' ), '', $fathom_url );
}
...
function fathom_get_custom_domain() {
	...
	$customDomainOption = 'https://' . str_replace( array(
			'https://',
			'http://'
		), '', get_option( FATHOM_CUSTOM_DOMAIN_OPTION_NAME, 'https://cdn.usefathom.com/' ) );

	// If we have a host
	if ( array_key_exists( 'host', (array) parse_url( $customDomainOption ) ) ) {
		$parsedUrl = wp_parse_url( $customDomainOption );
...
function fathom_print_js_snippet() {
	...
	<?php if ( empty( fathom_get_custom_domain() ) ): ?>
        ...
        <script src="https://cdn.usefathom.com/script.js" data-site="<?php echo esc_attr( $site_id ); ?>" defer></script>
        <!-- / Fathom -->
	<?php else: ?>
        <script src="https://<?php echo str_replace( [
			'http://',
			'https://',
			'/'
		], '', esc_attr( fathom_get_custom_domain() ) ); ?>/script.js" data-site="<?php echo esc_attr( $site_id ); ?>" defer></script>
	<?php endif; ?>
	<?php
}
...
function fathom_print_stats_page() {
	if ( empty( fathom_get_site_id() ) ) {
		echo '<div class="wrap">' . __( 'You have not configured Fathom. Go to Settings -> Fathom Analytics to configure this page. ', 'fathom-analytics' ) . '</div>';
	...
	echo '<div class="wrap">';
	echo '<iframe id="fathom-stats-iframe" src="https://app.usefathom.com/share/' . esc_attr( fathom_get_site_id() ) . '/wordpress?password=' . hash( 'sha256', get_option( FATHOM_PRIVATE_SHARE_PASSWORD ) ) . '" style="width: 1px;min-width: 100%; height:1000px; max-width:1100px" frameborder="0" onload=fathomResizeIframe();></iframe>';
	echo '</div>';
}
...
function fathom_print_settings_page() {
	echo '<div class="wrap">';
	echo sprintf( '<form method="POST" action="%1$s">', esc_attr( admin_url( 'options.php' ) ) );
...
function fathom_print_admin_tracking_setting_field( $args = array() ) {
	$value = fathom_get_admin_tracking();
