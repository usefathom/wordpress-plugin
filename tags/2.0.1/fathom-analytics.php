...
function fathom_get_custom_domain() {
  return get_option( FATHOM_CUSTOM_DOMAIN_OPTION_NAME, '' );
}
...
function fathom_print_js_snippet() {
  ...
  <?php if (empty(fathom_get_custom_domain())): ?>
   ...
   })(document, window, 'https://<?php echo str_replace(['http://', 'https://', '/'], '', esc_attr( $url )); ?>/tracker.js', 'fathom');
   fathom('set', 'siteId', '<?php echo esc_attr( $site_id ); ?>');
   fathom('trackPageview');
   </script>
   <?php else: ?>
    <script src="https://cdn.usefathom.com/3.js" site="<?php echo esc_attr( $site_id ); ?>"></script>
    <script>
        window.fathom || document.write('<script src="https://<?php echo str_replace(['http://', 'https://', '/'], '', fathom_get_custom_domain()); ?>/<?php echo substr(sha1(esc_attr($site_id)), 0, 6); ?>.js"><\/script>');
    </script>
   <?php endif; ?>
   <!-- / Fathom -->
   <?php
...
function fathom_print_url_setting_field( $args = array() ) {
   ...
   $placeholder = 'my-stats.usefathom.com/';
   ...
   echo '<p class="description">' . __( 'Do not edit this value unless you are using Fathom Lite', 'fathom-analytics' ) . '</p>';
}
